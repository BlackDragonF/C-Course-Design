...
void on_media_add_clicked(GtkWidget * widget , gpointer data)
{
    ...
    gtk_entry_set_max_length(GTK_ENTRY(dateentry), 11);
    gtk_entry_set_max_length(GTK_ENTRY(nameentry), 10);
    gtk_entry_set_max_length(GTK_ENTRY(indexentry), 50);
	...
	ProvinceNode province_node = province_with_name(head, gtk_combo_box_get_active_text(GTK_COMBO_BOX(provincecombo)));
    CaseNode case_node = NULL;
	if (province_node) case_node = province_node->case_head;
